...
| photomosaic 0.3.1 (CIELAB)        |  2.3 s |  0.065 |   0.776 |    36.41 |       0.111 |
| mosaicraft `fast`                 | 17.5 s |  0.228 |   0.613 |    10.17 |       0.321 |
| mosaicraft `vivid`                | 23.9 s |  0.387 |   0.421 |     8.72 |       0.323 |
| mosaicraft `vivid --cv 4`         | 86.5 s |  0.365 |   0.430 |     8.25 |   **0.338** |

SSIM and ΔE2000 reward pixel fidelity, which structurally favours mean-matching tools that reuse the same tiles. LPIPS (Zhang et al., CVPR 2018) correlates better with human judgement. Cell diversity counts the number of visually distinct cells (5-bit-quantised mean colour) — at 0.32 the same Vermeer that looks like a flat halftone in `codebox` (0.08) is built from ~515 distinct photos out of 1,600 cells.

`vivid --cv 4` edges out plain `vivid` on bucket diversity (0.338 vs 0.323) and ΔE2000 (8.25 vs 8.72) by giving the cost-matrix 4× hue-rotated colour matches — more cells get colour-closer tiles *and* more distinct tiles. Plain `vivid` is a hair sharper perceptually (LPIPS 0.421 vs 0.430). Either works; `--cv 4` is slower.
